...
import bgImg from "../images/register_bg.png";
...
const RegisterPage = () => {
  ...
  const router = useRouter();
    ...
    try {
      const { token, user, success, message } = await signin(username, email, password);
  ...
  return (
    ...
    <form className="w-[320px]" onSubmit={handleRegister}>
      <h2 className="text-3xl font-bold uppercase mb-6 text-black dark:text-white">Регистрация</h2>
      <FloatingInput
        icon={FaUser}
        placeholder="Имя"
        value={username}
